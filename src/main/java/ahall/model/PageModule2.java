...
public class PageModule2 {
	...
	public static String makePage(String pageName, int totalCnt, int listSize, int pageSize, int cp, String type) {
		...
		sb.append("<li class='page-item'>"
				+ "<a class='page-link' href='"+pageName+"?type="+type+"&cp=1'>&laquo;</a></li>");
		if(userGroup!=0){
			sb.append("<li class='page-item'>"
					+ "<a class='page-link' href='"+pageName+"?type="+type+"&cp="+previousCp
					+"'>&lt;</a></li>");
		}else{
		...
		for(int i=pageStart;i<=pageEnd;i++){
			if(cp==i){
				sb.append("<li class='page-item active'><a class='page-link' "
						+ "href='"+pageName+"?type="+type+"&cp="+i+"'>"+i+"</a></li>");
			}else{
		...
		if(userGroup!=pageCntGroup){
			sb.append("<li class='page-item'>"
					+ "<a class='page-link' href='"+pageName+"?type="+type+"&cp="+nextCp
					+"#reviewTab'>&gt;</a></li>");
		}else{
			sb.append("<li class='page-item disabled'>"
					+ "<a class='page-link' href='"+pageName+"?type="+type+"&cp="+nextCp
		...
		sb.append("<li class='page-item'>"
				+ "<a class='page-link' href='"+pageName+"?type="+type+"&cp="+pageCnt+"'>&raquo;</a></li>");
		sb.append("</ul></div>");
