...
<script>
	...
	$(function() {
		...
		$('#domestic-ea-adult').change(
				function() {
					if ((adult + children + infant) > 9) {
						alert('총 예약 가능한 좌석 수는 9석 입니다.');
						$('#domestic-ea-adult').find('option:first').attr(
								'selected', 'selected');
					}
				});
		$('#domestic-ea-children').change(
				function() {
					if ((adult + children + infant) > 9) {
						alert('총 예약 가능한 좌석 수는 9석 입니다.');
						$('#domestic-ea-children').find('option:first').attr(
								'selected', 'selected');
					}
				});
		$('#domestic-ea-infant').change(
				function() {
					if ((adult + children + infant) > 9) {
						alert('총 예약 가능한 좌석 수는 9석 입니다.');
						$('#domestic-ea-infant').find('option:first').attr(
								'selected', 'selected');
					}
				});
	})
					 ...
					 -->
						...
						<tr>
							<td
								...
								colspan="2"><input type="text" id="datepicker" value="가는날"
								readonly="readonly"> <input type="text" id="datepicker2"
								value="오는날" readonly="readonly"></td>
							<!-- <tr>
